...
fun `flow transformation should map correctly`() = runTest {
    ...
    repository.userFlow.test {
        ...
        assertTrue(user is User) // Domain model, not entity
    }
}
```
...
fun `load data should update from idle to loading to success`() = runTest {
    ...
    viewModel.stateFlow.test {
        assertEquals(LoadingState.Idle, awaitItem().loadingState)

        viewModel.loadData()

        assertEquals(LoadingState.Loading, awaitItem().loadingState)
        assertEquals(LoadingState.Success, awaitItem().loadingState)
    }
}
```
...
fun `load data with error should emit failure state`() = runTest {
    ...
    viewModel.stateFlow.test {
        ...
        assertEquals("Network error",
            (errorState.loadingState as LoadingState.Error).message)
    }
}
```
