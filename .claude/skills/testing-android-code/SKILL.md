...
1. **assertCoroutineThrows + runTest** - Never wrap in `runTest`; call directly
2. **Static mock cleanup** - Always `unmockkStatic()` in `@After`
3. **StateFlow vs EventFlow** - StateFlow: `awaitItem()` first; EventFlow: `expectNoEvents()` first
4. **FakeDispatcherManager** - Always use instead of real `DispatcherManagerImpl`
5. **Coroutine test wrapper** - Use `runTest { }` for all Flow/coroutine tests
