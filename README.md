...
2. Create a `user.properties` file in the root directory of the project and add the following properties:
    ...
    - `localSdk`: A boolean value to determine if the SDK should be loaded from the local maven artifactory. This is particularly useful when developing new SDK capabilities (ex: `localSdk=true`). Review [Linking SDK to clients](https://contributing-docs.pages.dev/getting-started/sdk/#linking-sdk-to-clients) for more details.
