...
class BiometricsEncryptionManagerImpl(
    ...
    override fun isBiometricIntegrityValid(cipher: Cipher?): Boolean =
        isSystemBiometricIntegrityValid(cipher) && isAccountBiometricIntegrityValid()
     ...
     */
    private fun isSystemBiometricIntegrityValid(cipher: Cipher?): Boolean {
        // Attempt to get the user scoped key. If that fails, we check to see if a legacy key
        // is available.
        val secretKey = getSecretKeyOrNull() ?: getSecretKeyOrNull()
