...
class SnackbarRelayManagerTest {
    ...
    @Test
    fun `When no relay is specified by producer, defaults to send data to the most recent relay registered`() =
        runTest {
            val relayManager = SnackbarRelayManagerImpl()
            val relay1 = SnackbarRelay.create()
            val relay2 = SnackbarRelay.create()
            val expectedData = BitwardenSnackbarData(message = "Test message".asText())
            turbineScope {
