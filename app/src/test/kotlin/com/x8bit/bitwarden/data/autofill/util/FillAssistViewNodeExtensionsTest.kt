...
class FillAssistViewNodeExtensionsTest {
    ...
    fun `buildFillAssistViews should map all identity field keys to correct AutofillView subtypes`() {
        val identityFieldExpectations: List<Pair<String, AutofillView>> = listOf(
            "fullName" to AutofillView.Identity.PersonNameFull(data = autofillData()),
            "honorificPrefix" to AutofillView.Identity.PersonNamePrefix(data = autofillData()),
            "firstName" to AutofillView.Identity.PersonNameGiven(data = autofillData()),
    ...
    fun `buildFillAssistViews should exclude identity field key when IdentityAutofill is disabled`() {
        ...
        val hostRule = FillAssistRules.HostRule(
            category = "identity-form",
            fields = mapOf(
                "fullName" to listOf(selectorClause(tag = "input", id = "name")),
            ),
        )
    ...
    fun `buildFillAssistViews should prefer Login Email over Identity PersonNameFull when identity key is listed first and matches the same node`() {
        ...
        val hostRule = FillAssistRules.HostRule(
            category = "account-login",
            fields = linkedMapOf(
                "fullName" to listOf(selectorClause(tag = "input", id = "shared")),
                "email" to listOf(selectorClause(tag = "input", id = "shared")),
            ),
        )
