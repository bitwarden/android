...
class AddEditSendViewModelTest : BaseViewModelTest() {
    ...
    fun `RegeneratePassword updates password in viewState`() = runTest {
        ...
        when (val viewState = viewModel.stateFlow.value.viewState) {
            is AddEditSendState.ViewState.Content -> {
                assertEquals(generatedPassword, viewState.common.passwordInput)
            }

            else -> {
                val actualType = viewModel.stateFlow.value.viewState::class.simpleName
                throw AssertionError(
                    "Expected viewState to be Content but was $actualType",
                )
            }
        }
    }
