...
class AutoFillScreenTest : BitwardenComposeTest() {
    ...
    fun `when browser autofill options are clicked the correct action is sent`() {
        ...
        verify(exactly = 1) {
            viewModel.trySendAction(
                AutoFillAction.ChromeAutofillSelected(BrowserReleaseChannel.CHROME_BETA),
            )
            viewModel.trySendAction(
                AutoFillAction.ChromeAutofillSelected(BrowserReleaseChannel.CHROME_STABLE),
            )
        }
    }
    ...
    fun `when NavigateToBrowserAutofillSettings events are sent they invoke the intent manager with the correct release channel`() {
        mutableEventFlow.tryEmit(
            AutoFillEvent.NavigateToBrowserAutofillSettings(
                BrowserReleaseChannel.CHROME_STABLE,
            ),
        )
        mutableEventFlow.tryEmit(
            AutoFillEvent.NavigateToBrowserAutofillSettings(
                BrowserReleaseChannel.CHROME_BETA,
            ),
        )

        verify(exactly = 1) {
            intentManager.startChromeAutofillSettingsActivity(BrowserReleaseChannel.CHROME_BETA)
            intentManager.startChromeAutofillSettingsActivity(BrowserReleaseChannel.CHROME_STABLE)
        }
    }
