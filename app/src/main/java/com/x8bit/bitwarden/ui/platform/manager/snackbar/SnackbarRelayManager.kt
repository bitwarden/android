...
interface SnackbarRelayManager {
    /**
     * Called from a producer to send snackbar data to a consumer, the producer can
     * specify the [relay] to send the data to, or can leave it null and it will be
     * sent to the "most recently active" consumer if available.
     */
    fun sendSnackbarData(data: BitwardenSnackbarData, relay: SnackbarRelay? = null)
