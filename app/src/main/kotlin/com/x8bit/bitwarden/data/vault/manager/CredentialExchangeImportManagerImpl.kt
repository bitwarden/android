...
class CredentialExchangeImportManagerImpl(
    ...
    private val syncManager: VaultSyncManager,
        ...
        .flatMap { importCiphersResponseJson ->
            when (importCiphersResponseJson) {
                ...
                ImportCiphersResponseJson.Success -> {
                    syncManager
                        .sync(userId = userId, forced = true)
                        .asSuccess()
                }
            }
        }
        .map { syncVaultDataResult ->
            when (syncVaultDataResult) {
                is SyncVaultDataResult.Error -> {
                    ImportCxfPayloadResult.SyncFailed(
                        error = syncVaultDataResult.throwable,
                    )
                }
