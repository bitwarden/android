 ...
 *
 * Visibility for the account number and PIN fields is managed locally by the composable via
 * [rememberSaveable] because the canonical [VaultItemState.ViewState.Content.ItemType.BankAccount]
 * carries plain string values rather than visibility-wrapped data. The corresponding ViewModel
 * actions remain dispatched so future telemetry (e.g. organization events) can hook in without a
 * UI change.
 */
@Suppress("LongMethod")
@Composable
        ...
        bankAccountState.accountNumber?.let { accountNumber ->
            item(key = "accountNumber") {
                ...
                BitwardenPasswordField(
                    ...
                    showPasswordChange = {
                        showAccountNumber = it
                        vaultBankAccountItemTypeHandlers.onAccountNumberVisibilityClick(it)
                    },
                    readOnly = true,
                    singleLine = false,
                    actions = {
        ...
        bankAccountState.pin?.let { pin ->
            item(key = "pin") {
                ...
                BitwardenPasswordField(
                    ...
                    showPasswordChange = {
                        showPin = it
                        vaultBankAccountItemTypeHandlers.onPinVisibilityClick(it)
                    },
                    readOnly = true,
                    singleLine = false,
        ...
        bankAccountState.iban?.let { iban ->
            item(key = "iban") {
                BitwardenTextField(
                    ...
                    textFieldTestTag = "BankAccountItemIbanEntry",
                    cardStyle = CardStyle.Middle(),
                    modifier = Modifier
                        .fillMaxWidth()
                        .standardHorizontalMargin()
