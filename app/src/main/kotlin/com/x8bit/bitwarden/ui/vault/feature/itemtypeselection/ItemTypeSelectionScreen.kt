...
/**
 * Screen that lets the user choose which type of vault item to create.
 *
 * Displayed in place of the legacy "New" dropdown when the new item types feature flag is enabled.
 */
...
fun ItemTypeSelectionScreen(
    ...
    EventsEffect(viewModel = viewModel) { event ->
        when (event) {
            ...
            is ItemTypeSelectionEvent.NavigateToAddItem -> onNavigateToAddItem(event.cipherType)
        }
    }
    ...
    BitwardenScaffold(
        ...
        LazyColumn(
            modifier = Modifier
                .fillMaxSize()
                .testTag(tag = "ItemTypeSelectionList"),
        ) {
            item(key = "top_spacer") {
                ...
                BitwardenGroupItem(
                    ...
                    onClick = {
                        viewModel.trySendAction(
                            ItemTypeSelectionAction.ItemTypeClick(cipherType = option.cipherType),
                        )
                    },
                    cardStyle = state.itemTypes.toListItemCardStyle(
                        index = index,
